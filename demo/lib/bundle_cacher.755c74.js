...
function unzipEntry(n, s, a, o) {
    ...
    n.forEach((e, t) => {
        if (t.dir) {
            ...
            i.push(t.async("arraybuffer").then(e => {
...
BundleAsyncCacher.prototype.onMessage = async function (n) {
    if (n.data.what == "installBundle") {
        ...
        if (e) {
            ...
            await this.installBundle(n.source, n.data.bundle, n.data.version).catch(e => {
...
BundleAsyncCacher.prototype.installBundle = async function (t, n, s) {
    const e = n/*  + "." + s */;
    const a = this.fetchOrigin + this.rootRoute + e + ".zip";
    ...
    while (true) {
        const { done: g, value: B } = await i.read();
        if (g) {
            break
        }
        c.set(B, l);
...
BundleAsyncCacher.prototype.onFetch = async function (n, e) {
    ...
    for (let [e, t] of this.cachedBundles) {
        ...
        if (s >= 0) {
            return this.openCache(t).then(e => {
                return e.match(n.url).then(e => {
                    if (e) {
                        return e.clone()
                    }
                    return fetch(a)
    ...
    if (this.fetchOrigin.length) {
        if (n.url.indexOf("assets") >= 0) {
            var s = n.url.indexOf(this.rootRoute);
            return fetch(this.fetchOrigin + n.url.substring(s))
        }
    }
    return fetch(n)
}
